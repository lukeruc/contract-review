...
如未明确指定，我将默认：代表乙方，平衡风险偏好，全面审核。
```

**5.3 记录审核立场**

用户确认后，将审核立场记入 `_internal/architect-materials/shared-context.md`（简单模式下也创建此文件的最小版本，仅含审核立场部分）。所有后续 Task Agent 的 task spec 中均应包含此审核立场信息。
