...
`find` 字段是 docx-mcp `search_text` 的搜索关键词——必须选文本中最独特的片段（数字、专有名词优先），15-40 字，避免通用开头如"本合同""双方"。
...
docx-mcp 依赖 `w14:paraId` 定位段落。旧版 Word 生成的合同可能缺失此属性，须在修订前补充：
...
### 3.4 创建 Revision Task Agent（T-REV）

阅读 `references/task-revision.md` 参考示例了解典型结构，然后自行撰写。

**注入工具**：`docx-mcp`（MCP 原生工具，不需 Bash）。关键操作流程：

```
open_document → 逐条处理 revisions.json：
  search_text(find) 定位段落 → get_paragraph(paraId) 核实原文
  → delete_text(paraId, old) [modify/delete 型]
  → insert_text(paraId, new) [modify/insert_after 型]
  → add_comment(paraId, comment)
→ audit_document() → save_document(output_path)
```

**注入文件**：
- `_internal/task-records/T-TRN/revisions.json`（操作手册）
- `_internal/architect-materials/shared-context.md`（含修订人姓名，注入为 docx-mcp 的 `author` 参数）

**下达任务**：创建 Task Agent，按 revisions.json 逐条在 .docx 上执行修订。产出 `output/{原合同名}-revised.docx`。

### 3.5 Architect 验收修订稿

用 Word 打开验收：修订标记是否清晰、批注是否正确、有无误改或遗漏。不通过则修正 revisions.json 后从 3.4 重新执行。
...
已于阶段 3.4 生成 `output/{原合同名}-revised.docx`，无需额外步骤。
...
- {原合同名}-revised.docx — 修订后合同（T-TRN revisions.json + T-REV docx-mcp 执行生成）
