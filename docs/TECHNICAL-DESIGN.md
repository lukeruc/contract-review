...
| 工具名 | 用途 | 状态 |
|--------|------|------|
| `char-count` | 获取纯文本字符数，用于 10000 字符阈值建议 | 已开发 |
...
[ bootstrap: 格式转化 → contract.md，原始文件 → original/ ]
       │
       ▼
[ 获取字符数，在信息确认步骤向用户建议模式。用户选择简单模式 ]
       │
       ▼
[ 信息确认：暂停流程请用户确认模式选择、代表方、审核目标、风险偏好、修订人姓名 ]
       │
       ▼
...
[ T-002 调用 docx 工具：修订 Markdown → output/{原合同名}-revised.docx（修订模式，每处修订附批注）]
       │
       ▼
[ 交付：audit-opinion.md + audit-work-report.md/.html + {原合同名}-revised.docx ]
       │
       ▼
...
```
[ 获取字符数，在信息确认步骤向用户建议模式。用户选择复杂模式 ]
       │
       ▼
[ 信息确认：暂停流程请用户确认模式选择、代表方、审核目标、风险偏好、修订人姓名、初设后是否自动继续 ]
       │
       ▼
...
[ Format Task Agents ]
  ...
  - T-FMT-01：修订 Markdown + original/ 原始文件 → output/{原合同名}-revised.docx（修订模式，每处修订附批注）
       │
       ▼
[ 最终交付：preliminary-report.md/.html + audit-opinion.md + audit-work-report.md/.html + {原合同名}-revised.docx ]
       │
       ▼
[ Architect 编制 agent/ 对话续接文件 ]
